-- 1) Make withdraw_to_mpesa store amount=principal, fee separate
...
BEGIN
  ...
  INSERT INTO public.transactions (user_id, type, amount, currency, fee, status, description)
    VALUES (_user, 'withdrawal', _amount, 'KES', _fee, 'pending',
...
BEGIN
  ...
  INSERT INTO public.transactions (user_id, type, amount, currency, fee, status, description)
    VALUES (_user, 'transfer_out', _amount, 'KES', _fee, 'pending',
...
-- 3) Lower the absurd flat fees
UPDATE public.app_config SET value = '{"flat": 5, "percent": 1}'::jsonb WHERE key = 'fee_withdraw_mpesa';
UPDATE public.app_config SET value = '{"flat": 5, "percent": 1}'::jsonb WHERE key = 'fee_send_mpesa';